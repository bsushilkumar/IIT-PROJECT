...

			{
				try {
					...
					for (int i = 0; i < startkm1FieldList.size(); i++) {
						...
						block.setStartMilePost(Double.parseDouble(startkm1FieldList.get(i).getText()));
						block.setEndMilePost(Double.parseDouble(endkm1FieldList.get(i).getText()));

						int blockNo = UpBlockIdList.get(i);
						block.setBlockNo(blockNo);
						block.setMaximumPossibleSpeed(Double.parseDouble(BlockVelocity1FieldList.get(i).getText()));
						...
						if (!Speedrestrictionstartkm1FieldList.get(i).getText().isEmpty()
								&& !Speedrestrictionendkm1FieldList.get(i).getText().isEmpty()
								&& !Speedrestrictionmaxspeed1FieldList.get(i).getText().isEmpty()) {
							double speedLimit = Double.parseDouble(Speedrestrictionmaxspeed1FieldList.get(i).getText());
							double startKm = Double.parseDouble(Speedrestrictionstartkm1FieldList.get(i).getText());
							double endKm = Double.parseDouble(Speedrestrictionendkm1FieldList.get(i).getText());

							SpeedRestriction speedRestrictionFormat = new SpeedRestriction(speedLimit, startKm, endKm);
							...
							if (i == 0) {
								for (Loop loop : sectioninputdialog.loopList) {
									...
									if (loopstationId == blockstationId) {
										...
										int loopIdcheck = Integer.parseInt(loopId.substring(2, 3));
										...
										if (loopIdcheck != 1) {
											...
											int loopcrossovercheck = Integer.parseInt(loopId.substring(3, 5));

											if (loopcrossovercheck == 23) {
												link.crossoverIds.add(UpBlockIdList.get(i));
